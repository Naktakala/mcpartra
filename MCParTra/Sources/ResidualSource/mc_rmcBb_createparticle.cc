...
//                                                  0);
  ...
  new_particle.tally_method = mcpartra::TallyMethod::RMC_CHAR_RAY;

  if (collided_source_mode == CollidedSrcMode::DIRECT)
    new_particle.tally_mask = new_particle.tally_mask |
                              mcpartra::SourceDrivenSolver::TallyMask::MAKE_DIRECT_PARTICLES;
  ...
  CheckForReExecution()
{
  if (collided_source_mode == CollidedSrcMode::STAGGERED)
  {
    if (ray_trace_phase)
    {
      ...
      typedef mcpartra::SourceDrivenSolver::TallyMask maskv;
...
CreateCollidedParticle(chi_math::RandomNumberGenerator& rng)
{
  ...
  new_particle.ray_trace_method = mcpartra::RayTraceMethod::STANDARD;
  new_particle.tally_method = mcpartra::TallyMethod::STANDARD;
